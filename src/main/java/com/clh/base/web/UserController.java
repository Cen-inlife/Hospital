...
import com.clh.base.entity.*;
import com.clh.base.service.DepartService;
import com.clh.base.service.DoctorService;
import com.clh.base.service.UserService;
import com.clh.base.util.EncryptUtil;
...
public class UserController
{
    ...
    @ResponseBody
    public ResultUtil updateUser(User user,HttpServletRequest request)throws ParseException
    {
        ResultUtil resultUtil = userService.updateUser(user);
        int user_id = user.getUser_id();
        User userById = userService.getUserById(user_id);
        request.getSession().setAttribute("hospital_user",userById);
        return resultUtil;
    }
    ...
    public Map<String, Object> image(@RequestParam(value = "file") MultipartFile file, HttpServletRequest request, HttpServletResponse response)
    {
       ...
       // String realPath = request.getServletContext().getRealPath("");
        //String localPaths = "C:\\workSpace\\ssmHospital\\src\\main\\webapp\\upload\\"; //存放我们上传的头像*/
        String localPaths = "G:\\mavenProject\\src\\main\\webapp\\upload\\";
        try {
            if (file != null) {
                //生成uuid作为文件名称
...
//            判断是否有这个文件夹，没有就创建
                ...
                File file2 = new File(localPaths + filename);
//            复制到当前文件夹
